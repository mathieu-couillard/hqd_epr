...
def fit_2d_data(
    ...
    Q_int: float = 160677,
    Q_ext: float = 9120,
    g_ens: float = 2 * np.pi * 5e6,
    inhomo_broad: float = 2 * np.pi * 5e6,  # This might not be used
):
    omegas = freqs * (2 * np.pi) *1e9
    ...
    return curve_fit(reflection_2d_resonator_spin, x_data, y_data, p0=p0, ftol=1e-30)
...
if __name__ == "__main__":
    ...
    popt, pcov = fit_2d_data(
        freqs=freqs_GHz[freq_start_lim: freq_stop_lim],
        b_fields=b_fields,
    ...
    print(b_fields[0])
    plot(freqs_GHz, reflection_2d_resonator_spin((freqs_GHz*2*np.pi*1e9, b_fields[0]), *popt))
    plt.show()
