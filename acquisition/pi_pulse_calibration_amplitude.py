...
amplitude_list = np.linspace(amplitude_min, amplitude_max, n_amplitude)
...
with program() as pi_pulse_calibration_amplitude:
    ...
    with for_(*from_array(a, amplitude_list)):
    ...
    if simulate:
        ...
        while results.is_processing():
            ...
            plt.savefig(
                (
                    fig_path
                    / f"pulse_amplitude_{round(amplitude * QMConfig.spin_if_pi_amp * 1e6):06d}uV".replace(
                        ".", "p"
                    )
                )
            )
            ...
            df_phase = pd.DataFrame(phase, index=amplitude_list[: len(phase)], columns=t).T
