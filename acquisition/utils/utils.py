...
def iq_correction(IQ, A, a, b, theta, theta_q):
    if isinstance(IQ, list) or isinstance(IQ, np.ndarray):
        IQ = IQ[0] + 1j * IQ[1]
    IQ *= IQ * np.e ** (-1j * theta)
    Inoisy = IQ.real
    Qnoisy = IQ.imag
    I = Inoisy + a
    Q = A * (Qnoisy * np.cos(theta_q) - I * np.sin(theta_q)) + b
    return (I, Q)
...
def source_ramp(source, target_level, rate=3e-3, step_size=1e-4):
...
def log_space(start, stop, base):
    n = np.log(stop / start) / np.log(base)
    return start * base ** (np.arange(0, n, 1))
