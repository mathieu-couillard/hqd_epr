...
from drivers import KeysightPSG, AMI430Vector, KeysightN9010A
...
def create_directories(project, exp_name):
    path = generate_path(project=project, exp_name=exp_name)
    config_path = qm_config.__file__
    exp_config_path = experiment_config.__file__
    shutil.copy(__file__, f"{str(path / Path(__file__).name)}")
    shutil.copy(config_path, f"{str(path / Path(config_path).name)}")
    shutil.copy(exp_config_path, f"{str(path / Path(exp_config_path).name)}")
    return path
...
if simulate:
    ...
    # Set up microwave
    mw_source = KeysightPSG(InstAddr.mw_source)
