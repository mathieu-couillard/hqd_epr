...
def create_directories(project, exp_name, curr):
...
def main(current):
    ...
    project_name = "impedance_matching_dpph"
    print(current_str)
    experiment_name = "rutile_dpph_hahn_echo_gradient{}p{}mA".format(
    ...
    if simulate:
        ...
        source_ramp(mag_grad_source, MagConfig.gradient_current)
        ...
        path, data_path, fig_path = create_directories(
            project_name, experiment_name, current
        )
        ...
        while results.is_processing():
            ...
            progress_counter(len(I) - 1, n_amplitude, start_time=start_time)

            # Plot results
            plt.suptitle(
                f"Hahn echo for {round(amplitude * QMConfig.spin_if_pi_amp * 1e6):06d} uV pulses."
            )
            plt.subplot(211)
            plt.cla()
            ...
            plt.savefig(
                (
                    fig_path
                    / f"pulse_amplitude_{round(amplitude * QMConfig.spin_if_pi_amp * 1e3):06d}uV".replace(
                        ".", "p"
                    )
                )
            )
            ...
            # Save results
            df_I = pd.DataFrame(I, index=amplitude_list[: len(I)], columns=t).T
            df_I.to_csv((data_path / "I.csv"))
            df_Q = pd.DataFrame(Q, index=amplitude_list[: len(Q)], columns=t).T
            df_Q.to_csv((data_path / "Q.csv"))
            df_amp = pd.DataFrame(amp, index=amplitude_list[: len(amp)], columns=t).T
            df_amp.to_csv((data_path / "amp.csv"))
            df_phase = pd.DataFrame(
                phase, index=amplitude_list[: len(phase)], columns=t
            ).T
            df_phase.to_csv((data_path / "phase.csv"))
            print("\n")
            if round(amplitude, 3) == amplitude_list[-1]:
...
if __name__ == "__main__":
    current = MagConfig.gradient_current
    main(current)
