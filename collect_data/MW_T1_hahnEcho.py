...
experiment_name = 'MW_T1_HahnEcho_4K_DPPH'

chunk_size = 20//4
array_size = readout_length // (chunk_size*4)

tau_n_us = int(2) # in us
# tau_n_10us = int(1) # in 10us 
cd_n_ms = int(1) # in ms cooldown
# sat_n_10us = int(1) # in us

Tus_min = int(1) # in us
Tus_max = int(1e6) # in us
T_list = np.array(digit_array_generator(Tus_min, Tus_max))

with program() as T1_HahnEcho:
    qm_us = int(1e3//4)
    qm_ms = int(1e6//4)
    # qm_10us = int(1e4//4)
    ...
    with for_(qm_iteration, 0, qm_iteration<n_avg, qm_iteration+1):
        ...
        with for_each_(qm_n_T, T_list):
            ...
            logger.info("T = {} us was successfully measured.".format(qm_n_T))
            # cooldown
            save(qm_i, i_st)
            assign(qm_i, qm_i+1)
            align('Digitizer','CryoSw', 'spin')
            wait(qm_ms*cd_n_ms, 'spin')
    ...
    with stream_processing():
        qm_I_st.buffer(array_size).buffer(len(T_list)).average().save('I')
        qm_Q_st.buffer(array_size).buffer(len(T_list)).average().save('Q')
...
def initialize_experiment(project, exp_name):
    ...
    shutil.copy(__file__,r'{}/MW_T1_HahnEcho.py'.format(exp_path))
...
simulate = True
if simulate:
    simulation_time = 32000 //4 
    job = qmm.simulate(config, T1_HahnEcho, SimulationConfig(duration =simulation_time))
    ...
    job = qm.execute(T1_HahnEcho)
    res_handles  = fetching_tool(job, data_list=['Iteration', 'I', 'Q', 'T_count'], mode = 'live')
    
    
    while res_handles.is_processing():
        iteration, I , Q, T_count = res_handles.fetch_all()
        progress_counter(T_count, len(T_list),start_time=res_handles.get_start_time())
    ...
    save_data_2d_echo(np.array(T_list),t,I,Q,exp_path, 'T [us]', experiment_name)
    sg.mwIsOn = False
