...
if simulate:
    ...
    for i, curr in enumerate(curr_list):  # Loop over y-voltages
        ...
        # Save results
        df_I = pd.DataFrame(I, index=curr_list[:i+1], columns=t).T
        df_I.to_csv((data_path / "I.csv"))
        df_Q = pd.DataFrame(Q, index=curr_list[:i+1], columns=t).T
        df_Q.to_csv((data_path / "Q.csv"))
        df_amp = pd.DataFrame(amp, index=curr_list[:i+1], columns=t).T
        df_amp.to_csv((data_path / "amp.csv"))
        df_phase = pd.DataFrame(phase, index=curr_list[:i+1], columns=t).T
