...
# vna measurement configuration
    ...
    vna.traces_number(num = 8)
    vna.active_trace(1)
    vna.Spar(Spar = 'S12', trace= 1, chan= 1)
    vna.format_trace(trace_format = 'mlog')
    vna.active_trace(2)
    vna.Spar(Spar = 'S12', trace= 2, chan= 1)
    vna.format_trace(trace_format = 'phase')
    sleep(2)
    vna.active_trace(3)
    vna.Spar(Spar = 'S22', trace=3, chan=1)
    vna.format_trace(trace_format = 'mlog')
    vna.active_trace(4)
    vna.Spar(Spar = 'S22', trace=4, chan=1)
    vna.format_trace(trace_format = 'phase')
    sleep(2)
    vna.active_trace(5)
    vna.Spar(Spar = 'S32', trace=5, chan=1)
    vna.format_trace(trace_format = 'mlog')
    vna.active_trace(6)
    vna.Spar(Spar = 'S32', trace=6, chan=1)
    vna.format_trace(trace_format = 'phase')
    sleep(2)
    vna.active_trace(7)
    vna.Spar(Spar = 'S42', trace=7, chan=1)
    vna.format_trace(trace_format = 'mlog')
    vna.active_trace(8)
    vna.Spar(Spar = 'S42', trace=8, chan=1)
    vna.format_trace(trace_format = 'phase')
    sleep(5)
...
#variables
    ...
    frequency_list = [4.4] # GHz
    phase_offset_list = [0] # degree
    electrical_delay_list = [43.92]  # nano second
    span_list = [800] # MHz
    point_list = [10001] # required points in a span
    
    #power
    measurement_power = -10 # vna start power

    #average
    number_of_average = 10 # vna measurement averaging
...
# Experiment Initialization
    ...
    experiment_name = 'Transmission Line cross coupling inline 2'
    ...
    # collect data
        ...
        data.to_csv(r'{}/{}MHz_{}dBm.csv'.format(save_data_path,cfreq, _input_power), index=False)
